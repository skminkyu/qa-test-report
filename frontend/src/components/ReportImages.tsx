...
export default function ReportImages({ images, setImages, equipmentNotes, setEquipmentNotes, reportNo }: Props) {
  ...
  return (
    <div className="print-page bg-white border border-gray-400 p-6 mx-auto" style={{ maxWidth: 900 }}>
      ...
      {images.length === 0 ? (
        <div className="mt-4 border-2 border-dashed border-gray-300 p-8 text-center text-gray-400 text-sm">이미지가 없습니다. 위 버튼을 클릭하여 이미지를 추가하세요.</div>
      ) : (
          ...
          {images.map((img, idx) => (
            <div key={idx} className="border border-gray-300 rounded p-2">
              ...
              <div className="mt-2 flex gap-2 items-center">
                <input value={img.caption} onChange={e => setImages(images.map((im,i) => i===idx ? {...im, caption: e.target.value} : im))}
                  placeholder="캡션 입력 (예: 조리 전/시작)" className="flex-1 border border-gray-300 rounded px-2 py-1 text-sm outline-none" />
      ...
      <div className="mt-4 border border-gray-400">
        ...
        <textarea value={equipmentNotes} onChange={e => setEquipmentNotes(e.target.value)}
          rows={3} placeholder="사용장비를 입력하세요." className="w-full outline-none text-sm p-2 resize-none" />
      </div>
    </div>
  );
}
