...
export default function ReportForm({ reportId, onSaved }: Props) {
    ...
    if (reportId) {
      fetch(`/api/reports/${reportId}`).then(r => r.json()).then((data: Report) => {
        ...
        if (data.test_items && data.test_items.length > 0) {
          setRows(data.test_items);
          setDetailMethod(data.test_items[0]?.detail_method || '');
        }
  ...
  const handleTestItemSelect = (idx: number, itemName: string) => {
    ...
    setRows(prev => prev.map((r, i) => i === idx ? {
      ...
      test_method: found?.method || r.test_method,
      test_result: found?.default_result || r.test_result,
      test_standard: found?.standard || r.test_standard,
    } : r));
  };
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const rowsWithDetail = rows.map(r => ({ ...r, detail_method: detailMethod }));
      const body = {
        ...
        test_items: rowsWithDetail.filter(r => r.test_item_name),
        images,
      };
      let res;
      if (savedReportId) {
        res = await fetch(`/api/reports/${savedReportId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      } else {
        res = await fetch('/api/reports', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      }
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4 no-print">
        <button
          onClick={() => setPage(1)}
          className={`px-4 py-2 rounded font-medium text-sm ${page === 1 ? 'bg-red-700 text-white' : 'bg-white text-gray-700 border'}`}
        >
          Page 1 - 시험성적서
        </button>
        <button
          onClick={() => setPage(2)}
          className={`px-4 py-2 rounded font-medium text-sm ${page === 2 ? 'bg-red-700 text-white' : 'bg-white text-gray-700 border'}`}
        >
          Page 2 - 이미지
        </button>
        <div className="flex-1" />
        {savedReportNo && (
          <div className="bg-green-50 border border-green-300 rounded px-4 py-2 text-green-800 font-semibold text-sm">
            Report No: {savedReportNo}
          </div>
        )}
        <button onClick={handleReset} className="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded text-sm font-medium">초기화</button>
        <button onClick={handleSave} disabled={saving} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm font-medium disabled:opacity-50">
          {saving ? '저장 중...' : '저장'}
        </button>
        ...
        <div className="print-page bg-white border border-gray-400 p-6 mx-auto" style={{ maxWidth: 900 }}>
          ...
          <div className="flex items-stretch border border-gray-400 mb-0">
            ...
            <div className="flex-[2] flex items-center justify-center border-r border-gray-400">
              <div className="text-2xl font-bold tracking-widest text-gray-900">시 험 성 적 서</div>
            </div>
            <div className="flex-1 flex items-center justify-center text-sm text-gray-600 p-2">
              <div className="text-center">
                <div className="font-semibold">Page 1 / 2</div>
          ...
          <table className="w-full border-collapse border border-gray-400 text-sm">
            <tbody>
              <tr>
                ...
                <td className="border border-gray-400 p-1">
                  <input value={company} onChange={e => setCompany(e.target.value)} placeholder="업체명 입력"
                    className="w-full outline-none text-sm p-1" />
                </td>
                <td className="border border-gray-400 bg-gray-100 font-semibold p-2 w-24 text-center">Report No.</td>
                <td className="border border-gray-400 p-2 text-red-700 font-bold text-sm">
                  {savedReportNo || '저장 후 생성'}
                </td>
              </tr>
              <tr>
                ...
                <td className="border border-gray-400 p-1">
                  <select value={testerId} onChange={e => setTesterId(e.target.value ? Number(e.target.value) : '')}
                    className="w-full outline-none text-sm p-1">
          ...
          {/* Test items table */}
          <table className="w-full border-collapse border border-gray-400 text-sm mt-0">
            <thead>
            ...
            <tbody>
              {rows.map((row, idx) => (
                <tr key={idx}>
                  ...
                  <td className="border border-gray-400 p-1">
                    <select
                      value={row.test_item_name}
                      onChange={e => handleTestItemSelect(idx, e.target.value)}
                      className="w-full outline-none text-sm p-1"
                    >
                      <option value="">선택</option>
                  ...
                  <td className="border border-gray-400 p-1">
                    <input value={row.test_method} onChange={e => updateRow(idx, 'test_method', e.target.value)}
                      className="w-full outline-none text-sm p-1" placeholder="시험방법" />
                  </td>
                  <td className="border border-gray-400 p-1">
                    <input value={row.test_result} onChange={e => updateRow(idx, 'test_result', e.target.value)}
                      className="w-full outline-none text-sm p-1" placeholder="시험결과" />
                  </td>
                  <td className="border border-gray-400 p-1">
                    <input value={row.test_standard} onChange={e => updateRow(idx, 'test_standard', e.target.value)}
                      className="w-full outline-none text-sm p-1" placeholder="시험기준" />
                  </td>
                  <td className="border border-gray-400 p-1 text-center no-print">
                    <button onClick={() => removeRow(idx)} className="text-red-500 hover:text-red-700 font-bold text-sm">×</button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="no-print mt-1">
            <button onClick={addRow} className="text-sm text-blue-600 hover:text-blue-800 font-medium">+ 행 추가</button>
          </div>
              ...
              <div className="flex-1 border-r border-gray-400 p-3">
                ...
                {selectedTester ? (
                  <div className="text-center">
                    ...
                    {selectedTester.signature_image ? (
                      <img src={selectedTester.signature_image} alt="서명" className="h-12 mx-auto mt-2 object-contain" />
                    ) : (
                      <div className="h-12 border border-dashed border-gray-300 mt-2 flex items-center justify-center text-xs text-gray-400">서명 없음</div>
                    )}
                  </div>
                ) : (
              ...
              <div className="flex-1 p-3">
                ...
                {teamLeader ? (
                  <div className="text-center">
                    ...
                    {teamLeader.signature_image ? (
                      <img src={teamLeader.signature_image} alt="서명" className="h-12 mx-auto mt-2 object-contain" />
                    ) : (
                      <div className="h-12 border border-dashed border-gray-300 mt-2 flex items-center justify-center text-xs text-gray-400">서명 없음</div>
                    )}
                  </div>
